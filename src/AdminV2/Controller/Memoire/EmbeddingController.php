...
namespace ArnaudMoncondhuy\SynapseBundle\AdminV2\Controller\Memoire;
...
use ArnaudMoncondhuy\SynapseBundle\Core\Service\EmbeddingService;
use ArnaudMoncondhuy\SynapseBundle\Core\VectorStore\VectorStoreRegistry;
...
/**
 * Embeddings & RAG — Configuration du moteur sémantique — Admin V2
 */
#[Route('/synapse/admin-v2/memoire/embeddings', name: 'synapse_v2_admin_embeddings')]
class EmbeddingController extends AbstractController
{
    ...
    public function index(Request $request): Response
    {
        ...
        if ($request->isMethod('POST')) {
            $this->validateCsrfToken($request, $this->csrfTokenManager, 'synapse_v2_admin_embeddings');
            ...
            if ($embProvider && $embModel && !isset($embeddingModelsByProvider[$embProvider][$embModel])) {
                ...
                return $this->redirectToRoute('synapse_v2_admin_embeddings');
            }
        ...
        return $this->render('@Synapse/admin_v2/memoire/embeddings.html.twig', [
            'config'                      => $config,
    ...
    #[Route('/test', name: '_test', methods: ['POST'])]
    public function test(Request $request): JsonResponse
    {
        $this->denyAccessUnlessAdmin($this->permissionChecker);
        $this->validateCsrfToken($request, $this->csrfTokenManager, 'synapse_v2_admin_embeddings_test');
