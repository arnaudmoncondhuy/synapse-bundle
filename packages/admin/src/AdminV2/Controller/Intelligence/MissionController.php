...
/**
 * Gestion des missions d'agents — Admin V2
 *
 ...
 */
#[Route('/synapse/admin-v2/intelligence/missions', name: 'synapse_v2_admin_missions')]
class MissionController extends AbstractController
{
    ...
    public function index(): Response
    {
        ...
        return $this->render('@Synapse/admin_v2/intelligence/missions.html.twig', [
            'missions' => $missions,
        ]);
    }
    ...
    public function new(Request $request): Response
    {
        ...
        if ($request->isMethod('POST')) {
            $this->validateCsrfToken($request, $this->csrfTokenManager, 'synapse_v2_mission_edit');
            ...
            return $this->redirectToRoute('synapse_v2_admin_missions');
        }

        $presets = $this->presetRepo->findAll();
        $tones = $this->toneRepo->findAllOrdered();

        return $this->render('@Synapse/admin_v2/intelligence/mission_edit.html.twig', [
            'mission'  => $mission,
            'is_new'   => true,
            'presets'  => $presets,
            'tones'    => $tones,
    ...
    #[Route('/{id}/limite', name: '_limit', methods: ['POST'])]
    public function saveLimit(SynapseMission $mission, Request $request): Response
    {
        $this->denyAccessUnlessAdmin($this->permissionChecker);
        $this->validateCsrfToken($request, $this->csrfTokenManager, 'synapse_v2_mission_limit_' . $mission->getId());
        ...
        if ($action === 'delete' && $limit !== null) {
            ...
            return $this->redirectToRoute('synapse_v2_admin_missions_edit', ['id' => $mission->getId()]);
        }
    ...
    {
        $this->denyAccessUnlessAdmin($this->permissionChecker);
        $this->validateCsrfToken($request, $this->csrfTokenManager, 'synapse_v2_mission_toggle_' . $mission->getId());
    ...
    {
        $this->denyAccessUnlessAdmin($this->permissionChecker);
        $this->validateCsrfToken($request, $this->csrfTokenManager, 'synapse_v2_mission_delete_' . $mission->getId());
