...
class ProviderController extends AbstractController
{
    ...
    /**
     * Édition des credentials d'un provider (Admin V2)
     */
    ...
    public function edit(SynapseProvider $provider, Request $request): Response
    {
        ...
        if ($request->isMethod('POST')) {
            $this->validateCsrfToken($request, $this->csrfTokenManager, 'synapse_v2_provider_edit_' . $provider->getId());
            ...
            return $this->redirectToRoute('synapse_v2_admin_configuration_llm', ['tab' => 'fournisseurs']);
        }

        $client = $this->clientRegistry->getClientByProvider($provider->getName());

        return $this->render('@Synapse/admin_v2/intelligence/provider_edit.html.twig', [
            'provider' => $provider,
            'fields'   => $client->getCredentialFields(),
        ]);
    }
    ...
    /**
     * Test des credentials d'un provider (Admin V2)
     */
    ...
    {
        $this->denyAccessUnlessAdmin($this->permissionChecker);
        $this->validateCsrfToken($request, $this->csrfTokenManager, 'synapse_v2_provider_test_' . $provider->getId());
