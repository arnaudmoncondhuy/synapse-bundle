...
### [ ] Encryption obligatoire des credentials et conversations
**Statut** : ✅ À migrer  
**Ce que ça fait** : `encryption.key` devient obligatoire dans la config (plus de mode pass-through). 11 services passent en non-nullable. Commande `synapse:credentials:encrypt-all` pour migrer les credentials existants en clair.  
**Pourquoi migrer** : Sécurité. Credentials en clair = risque. 979/979 tests OK. **Attention** : les apps hôtes devront déclarer `SYNAPSE_ENCRYPTION_KEY` dans leur `.env.local` avant de migrer.

---

### [ ] Provider Anthropic
**Statut** : ✅ À migrer  
**Ce que ça fait** : Nouveau provider LLM pour l'API Anthropic (Claude). Fichiers non commités sur la branche dev.  
**Note** : contribution directe du user — à commiter séparément.
