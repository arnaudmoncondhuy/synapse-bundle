...
L'interface `EncryptionServiceInterface` est la sentinelle de la vie privée dans SynapseBundle. Elle permet de chiffrer automatiquement les conversations et les données sensibles en base de données.
...
*   **Transparence** : SynapseBundle appelle automatiquement ces méthodes via le `ConversationManager`. Vous n'avez pas à gérer le chiffrement manuellement dans vos contrôleurs.
