...
L'interface `LlmClientInterface` est le connecteur universel de SynapseBundle. C'est elle qui permet de dialoguer avec les différents fournisseurs d'IA (OpenAI, Gemini, Mistral, Ollama, etc.) en utilisant un langage commun.
...
*   **Credential Fields** : Les champs retournés par `getCredentialFields` apparaîtront automatiquement dans l'interface d'administration de SynapseBundle.
