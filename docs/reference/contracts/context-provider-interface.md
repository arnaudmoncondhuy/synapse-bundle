...
L'interface `ContextProviderInterface` est l'un des outils les plus puissants de SynapseBundle. Elle permet d'injecter dynamiquement des instructions système et des données métiers au tout début de chaque échange avec l'IA.
...
> [!TIP]
> **Chaînage** : SynapseBundle permet de configurer plusieurs providers. Les instructions de chacun seront concaténées automatiquement pour former le prompt final.
