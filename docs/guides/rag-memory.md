...
SynapseBundle supporte plusieurs modes de stockage via l'option `vector_store.default`.
...
```php
use ArnaudMoncondhuy\SynapseBundle\Core\Memory\MemoryManager;
use ArnaudMoncondhuy\SynapseBundle\Shared\Enum\MemoryScope;
...
Lors d'une conversation, SynapseBundle :
...
En plus du RAG classique (documents pré-indexés), SynapseBundle dispose d'un système de **mémoire conversationnelle** où le LLM peut proposer de retenir des informations importantes avec le consentement de l'utilisateur.
